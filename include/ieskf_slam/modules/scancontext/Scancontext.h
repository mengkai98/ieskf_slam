...
using SCPointType = IESKFSlam::Point;  // using xyz only. but a user can exchange the original bin
...
class SCManager : public IESKFSlam::ModuleBase {
   public:
    // SCManager();  // reserving data space (of std::vector) could be considered. but the
    // descriptor is lightweight so don't care.
    SCManager(const std::string &config_file_path, const std::string &prefix);
   ...
   public:
    ...
    double SC_DIST_THRES = 0.13;  // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60
